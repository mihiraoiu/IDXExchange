...
SOLD_FILE = DATA_DIR / "CRMLSSold.csv"

sold = pd.read_csv(SOLD_FILE, low_memory=False)

print(f"Loaded sold data: {sold.shape[0]:,} rows × {sold.shape[1]:,} columns")
...
sold = missing_value_report(sold, "sold")
numeric_summary(sold, "sold")
create_plots(sold, "sold")
extreme_outlier_report(sold, "sold")

sold.to_csv(
    DATA_DIR / "CRMLSSold_Residential.csv",
    index=False
)
...
print(
    "\nSold unmatched mortgage-rate rows:",
    sold_enriched["rate_30yr_fixed"].isna().sum()
)

sold_enriched.to_csv(
    DATA_DIR / "CRMLSSold_Residential_WithRates.csv",
    index=False
)

print("\nSold analysis complete.")